...
class CoreDataStack {
    ...
    func deleteCheck(withChName name: String, completion: @escaping () -> Void) {
    ...
    func deleteGift(withGGift name: String, completion: @escaping () -> Void) {

        let context = CoreDataStack.shared.persistentContainer.viewContext
        let fetchRequest: NSFetchRequest<GiftModel> = GiftModel.fetchRequest()

        fetchRequest.predicate = NSPredicate(format: "gGift == %@", name)
